...
ventasOrdenadas = sorted(ventas)
cantidadVentas = len(ventasOrdenadas)

if cantidadVentas % 2 != 0:
    posicionMedio = cantidadVentas // 2
    mediana = ventasOrdenadas[posicionMedio]

else:
    posicionMedio1 = cantidadVentas // 2 - 1
    posicionMedio2 = cantidadVentas // 2

    mediana = (ventasOrdenadas[posicionMedio1] + ventasOrdenadas[posicionMedio2]) / 2
...
for venta in ventas:
    cantidadRepeticiones = ventas.count(venta)

    if cantidadRepeticiones > mayorCantidad:
        mayorCantidad = cantidadRepeticiones
        moda = venta
